...
const initialMessages = [
    { id: 1, sender: 'bot', text: "Hello! How can I help you with your course today?", timestamp: new Date() },
]
...
export default function ChatUI() {
    ...
    useEffect(() => {
        if (isLoading) {
            setMessages(prevMessages => [...prevMessages, { id: 'typing', sender: 'bot', isTyping: true }]);
        } else {
                ...
                {messages.map((message) => (
                    <div key={message.id} className={`flex ${message.sender === 'user' ? 'justify-end' : 'justify-start'} mb-4`}>
                        <div className={`flex ${message.sender === 'user' ? 'flex-row-reverse' : 'flex-row'} items-start max-w-[70%]`}>
                            ...
                            <div className={`mx-2 ${message.sender === 'user' ? 'bg-blue-500 text-white' : 'bg-gray-200'} ${message.isTyping ? '' : 'p-3 rounded-lg'}`}>
            ...
            <div className="p-4 border-t">
                <form onSubmit={(e) => { e.preventDefault(); handleSendMessage(); }} className="flex items-center space-x-2">
                    <div className="relative">
                        <Label htmlFor="pdf-upload" className={`cursor-pointer p-2 rounded-lg hover:bg-gray-100 transition-colors ${isLoading ? 'opacity-50 cursor-not-allowed' : ''}`}>
